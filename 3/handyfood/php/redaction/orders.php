<?php
    ...
    foreach( $_SESSION['cart-list'] as $product){
        ...
        $goods_price = $finishresult['goods_price'];
        ...
        $mysql->query("INSERT INTO `orders` (`order_number`, `goods_id`, `goods_count`, `user_id`, `order_status`, `total_sum`, `order_date`) VALUES ('$order_number','$goods_id','$goods_count','$user_id','В отправке!', '$goods_sumprice_symbol','$date')");
        $mysql->query("UPDATE `goods` SET `goods_count` = `goods_count`-'$goods_count' WHERE `goods_id` = '$goods_id'");
