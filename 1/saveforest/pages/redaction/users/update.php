...
<body>
    ...
    <main>
        ...
        ?>
        <h2 class="text-center my-5">Изменение пользователя</h2>
        <form action="redact.php" method="POST" class="container d-flex flex-column justify-content-center form_update my-5 w-50">
            <p>Уникальный индификатор</p>
            <input class="my-3" type="text" name="id" value="<?=$user['id']?>" readonly>
            <p>Логин</p>
            <input class="my-3" type="text" name="login" value="<?=$user['login'] ?>">
            <p>Пароль</p>
            <input class="my-3" type="text" name="pass" value="<?=$user['pass'] ?>">
            <p>Имя</p>
            <input class="my-3" type="text" name="name" value="<?=$user['name'] ?>">
            <button type="submit" class="btn btn-success my-4">Изменить</button>
        </form>
        <?php
            else:
        ?>
